...
export function Kinetic({ project, portfolio, themeMode, onSetThemeMode, themeStyles }: Props) {
  ...
  const glassEffect = 'bg-background/80 backdrop-blur-sm border-b border-border sticky top-0 z-50';
  ...
  return (
    <div className="min-h-screen bg-background text-foreground font-sans selection:bg-primary selection:text-primary-foreground overflow-x-hidden">
      ...
      <main className="max-w-7xl mx-auto p-4 md:p-8 space-y-24 pb-24">
        ...
        {/* 4. PROJECTS GALLERY (3D TILT CARDS) */}
        <section className="space-y-12">
          <Reveal>
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 border-b border-border pb-6">
              <div>
                <h2 className="text-3xl md:text-5xl font-black mb-2">Featured Work</h2>
                <p className="text-muted-foreground">A selection of recent deployments</p>
              </div>
              <div className="hidden md:flex items-center gap-2 text-sm font-bold text-primary">
                <MousePointer2 className="w-4 h-4 animate-bounce" /> Hover for details
              </div>
            </div>
          </Reveal>
            ...
            {portfolio.showcases.map((project, idx) => (
              <Reveal key={project.id} delay={idx * 100} className="h-full">
                <Link
                  href={project.url}
                  target="_blank"
                  className="block h-full cursor-none-custom"
                >
                  <TiltCard
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 relative z-10">
                <div className="space-y-6">
                  <h2 className="text-4xl md:text-5xl font-black leading-tight">
                    Let's Build <br /> Something <span className="text-primary">Impossible.</span>
                  </h2>
                  <p className="text-xl text-muted-foreground">
                    Have a vision? I'm ready to help you realize it. Send me a signal and let's get
                    to work.
                  </p>
      ...
      <footer className="border-t border-border bg-muted/20 py-12 text-center text-sm text-muted-foreground">
        <div className="max-w-7xl mx-auto px-4 flex flex-col md:flex-row justify-between items-center gap-4">
          <p>
            © {new Date().getFullYear()} {portfolio.name}. Built with Kinetic Energy.
          </p>
