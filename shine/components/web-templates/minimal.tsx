...
export function Minimal({ portfolio, themeMode, onSetThemeMode, themeStyles }: Props) {
    ...
    <div className="min-h-screen bg-background text-foreground p-4 md:p-12 font-sans">
      <div className="max-w-6xl mx-auto space-y-12">
        ...
        <header className="flex flex-col md:flex-row gap-8 items-center justify-between border-b border-foreground/10 pb-10">
          <div className="flex gap-6 items-center w-full">
            ...
            <div className="space-y-1 flex-1">
              ...
              <p className="mt-4 text-sm text-foreground/70">{portfolio.introduction}</p>
            </div>
          </div>
        </header>
        ...
        <div className="flex flex-col md:flex-row gap-12">
          ...
          <div className="md:w-8/12 space-y-10">
            ...
            <section className="border-l-4 border-primary/50 pl-6">
              ...
              <p className="text-base leading-relaxed text-foreground/80">{portfolio.about}</p>
            </section>

            {/* WORK EXPERIENCE */}
            <section>
              <h2 className={minimalistHeaderClass + ' flex items-center gap-2'}>
                <Briefcase className="w-5 h-5" /> Experience
              </h2>
              <div className="space-y-8">
                {portfolio.work_experiences.map((exp) => (
                  <div key={exp.id} className="relative pl-8 border-l border-foreground/20 group">
                    ...
                    <div className="flex justify-between items-start mb-2">
                      ...
                      <Badge variant="secondary" className="text-xs opacity-80 whitespace-nowrap">
            ...
            <section>
              ...
              <div className="grid grid-cols-1 gap-6">
                {portfolio.showcases.map((project) => (
                  <Link key={project.id} href={project.url} target="_blank" className="group block">
                    <Card className={minimalistCardClass + ' border border-border'}>
                  ...
                  {socialLinks.map((link, i) => (
                    <div key={i} className="flex items-center gap-3">
                      ...
                      <Link
                        ...
                        className="hover:text-primary transition-colors"
                      >
                        {link.type.charAt(0).toUpperCase() + link.type.slice(1)} Profile
                      </Link>
                    </div>
                  ))}
                </div>
        ...
        <section className="py-12 border-t border-foreground/10 mt-12">
          ...
          <Card className={minimalistCardClass + ' max-w-3xl mx-auto p-6 md:p-8'}>
            {error && (
              <Alert
                variant="destructive"
        ...
        <footer className="text-center text-sm pt-8 text-muted-foreground">
          Built with a Minimalist approach. © {new Date().getFullYear()} {portfolio.name}.
        </footer>
      </div>
    </div>
  );
