          ...
          <div className="flex flex-col md:flex-row gap-2 md:items-center text-sm md:text-lg font-bold">
            {portfolio.job_title && (
              <span className="flex items-center gap-2">
                <Terminal className="w-4 h-4 md:w-5 md:h-5" /> {portfolio.job_title}
              </span>
            )}
            {portfolio.location && (
              <>
                ...
                <span className="flex items-center gap-2">
                  <MapPin className="w-4 h-4 md:w-5 md:h-5" /> {portfolio.location}
                </span>
              </>
            )}
          </div>
...
const AboutSection = ({ portfolio }: { portfolio: Portfolio }) => {
  ...
  return (
    <section className="bg-muted p-6 border-4 border-foreground relative">
      ...
      {portfolio.about && <p className="mt-4 text-muted-foreground">{portfolio.about}</p>}
    </section>
  );
};
        ...
        {portfolio.work_experiences.map((exp, idx) => (
          <div
            ...
            className={`p-6 border-4 border-foreground bg-card ${
              idx % 2 === 0 ? 'md:translate-x-2' : 'md:-translate-x-2 md:hover:translate-x-0'
          ...
          >
            ...
            {exp.about && (
              <p className="text-muted-foreground border-l-4 border-primary/50 pl-4">{exp.about}</p>
            )}
          </div>
        ))}
      </div>
          ...
          >
            <Card className={brutalCardClass + ' h-full flex flex-col'}>
              ...
              <CardContent className="p-6 flex-1 flex flex-col gap-4">
                {project.description && (
                  <p className="font-medium flex-1 line-clamp-3">{project.description}</p>
                )}
        ...
        <div className="grid md:grid-cols-2 gap-6 mt-6">
          <div className="space-y-4">
            <div className="space-y-2">
              ...
              <Input {...register('name')} placeholder="YOUR NAME" className={brutalInputClass} />
              {errors.name && (
            ...
            <div className="space-y-2">
              ...
              {errors.contact_number && (
                <p className="text-red-500 font-bold text-xs">{errors.contact_number.message}</p>
              )}
            </div>
          </div>
