            ...
            {portfolio.work_experiences.map((exp) => (
              <div key={exp.id} className="grid grid-cols-12 gap-2 group">
                <div className="col-span-3 text-right pr-3 border-r border-foreground/20">
                  <p className="font-sans text-xs font-bold">{exp.start_date || 'N/A'}</p>
              ...
              <div key={proj.id} className="pt-3 first:pt-0">
                ...
                >
                  <h4 className="font-bold text-base leading-tight group-hover:underline decoration-2 underline-offset-2">
                    {proj.name}
                    {proj.url && <ArrowUpRight className="inline w-3 h-3 ml-1 mb-1 opacity-50" />}
                  </h4>
                  {proj.description && (
      ...
      <div className="border-4 border-double border-foreground p-3 mt-8 bg-background">
        <div className="text-center border border-foreground p-3">
          ...
          <div className="space-y-2 text-left">
            ...
            >
              {isLoading ? <Loader2 className="h-3 w-3 animate-spin" /> : 'Submit Query'}
            </Button>
          </div>
        </div>
      </div>
      ...
      <div className="flex flex-col items-center gap-4">
        ...
        <div className="flex flex-col items-center gap-2 pt-2">
          ...
          <div className="flex items-center gap-3 mt-1">
            <a href={githubURL} target="_blank" rel="noreferrer" className="hover:text-primary">
              <Github className="w-3 h-3" />
            </a>
            <a href={rootURL} target="_blank" rel="noreferrer" className="hover:text-primary">
              <Globe className="w-3 h-3" />
            </a>
            <a href={facebookURL} target="_blank" rel="noreferrer" className="hover:text-primary">
...
export function Newspaper({ project, portfolio, themeMode, onSetThemeMode }: Props) {
    ...
    <div className="min-h-screen bg-background text-foreground p-4 md:p-8 font-serif">
      <div className="max-w-6xl mx-auto space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 md:gap-6">
          ...
          <RightColumn portfolio={portfolio} project={project} projectId={project.id} />
        </div>
