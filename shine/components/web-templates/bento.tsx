...
const AboutCard = ({ portfolio, className, style }: SectionProps) => {
  ...
  return (
    ...
    >
      ...
      <CardContent>
        <p className="text-foreground/80 leading-relaxed text-lg">{portfolio.about}</p>
      </CardContent>
    </Card>
  );
};
...
const SkillsCard = ({ portfolio, className, style }: SectionProps) => {
  ...
  return (
    <Card
      className={cn('border-0 shadow-sm animate-in fade-in slide-in-from-bottom-4', className)}
      style={style}
    >
      <CardHeader>
...
const ExperienceEducationSection = ({ portfolio, className, style }: SectionProps) => {
  return (
    <div className={className} style={style}>
      {/* Experience */}
...
const ContactFormSection = ({
  ...
  return (
    <Card
      className={cn('border-0 shadow-xl overflow-hidden ring-1 ring-border/50', className)}
      style={style}
    >
...
export function Bento({ project, portfolio, themeMode, onSetThemeMode }: Props) {
  const isDarkMode = useMemo(() => themeMode === 'dark', [themeMode]);

  // Animation delay helper
  const delay = (ms: number) => ({ animationDelay: `${ms}ms`, animationFillMode: 'both' as const });
    ...
    <div className="min-h-screen bg-background text-foreground p-4 md:p-8 font-sans transition-colors duration-500">
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-4 auto-rows-[minmax(180px,auto)]">
          ...
          {/* 2. Contact Info */}
          <ContactInfoCard portfolio={portfolio} className="md:col-span-1" style={delay(100)} />

          {/* 3. Socials */}
          <SocialsCard portfolio={portfolio} className="md:col-span-1" style={delay(150)} />
