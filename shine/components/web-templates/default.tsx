        ...
        {portfolio.education.map((edu) => (
          ...
          >
            <div>
              ...
              {edu.degree && <div className="text-sm text-muted-foreground">{edu.degree}</div>}
            </div>
  ...
  return (
    <section className="space-y-4 rounded-xl bg-muted/30 p-6 border border-border/50">
      ...
      <div className="space-y-3">
        ...
        <Button onClick={handleSubmit(onSubmit)} disabled={isLoading} className="w-full mt-2">
...
const FooterSection = ({ portfolio }: { portfolio: Portfolio }) => {
  ...
  const githubURL = process.env.NEXT_PUBLIC_KISLAP_GITHUB_URL || 'https://kislap.app';
  const facebookURL = process.env.NEXT_PUBLIC_KISLAP_FACEBOOK_URL || 'https://kislap.app';
...
export function Default({ project, portfolio, themeMode, onSetThemeMode }: Props) {
  ...
  return (
    <div className="min-h-screen bg-background text-foreground selection:bg-primary/10 selection:text-primary font-sans">
      <div className="max-w-5xl mx-auto px-4 sm:px-6 py-10 sm:py-20 space-y-16 sm:space-y-20">
        <HeroSection portfolio={portfolio} themeMode={themeMode} onSetThemeMode={onSetThemeMode} />
