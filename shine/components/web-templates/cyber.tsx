...
const IdCardSection = ({ portfolio }: { portfolio: Portfolio }) => {
  const expYears = useMemo(() => {
    if (!portfolio.work_experiences || portfolio.work_experiences.length === 0) return 'N/A';
    ...
    <div className={cyberPanelClass + ' p-6 flex flex-col items-center text-center'}>
      ...
      <div className="grid grid-cols-2 gap-2 w-full mt-6">
        <div className="bg-primary/5 p-2 text-center border border-primary/10">
          ...
          <div className="text-xl font-bold font-mono">{portfolio.showcases?.length || 0}</div>
        </div>
        <div className="bg-primary/5 p-2 text-center border border-primary/10">
...
const ContactFormSection = ({ portfolio, project }: { portfolio: Portfolio; project: Project }) => {
...
const AboutSection = ({ portfolio }: { portfolio: Portfolio }) => {
  ...
  return (
    <section className="relative pl-6 border-l border-primary/20">
      ...
      <h2 className="text-2xl font-black uppercase mb-4 tracking-tight">System_Log // About</h2>
    ...
    <section>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {portfolio.showcases.map((project) => (
          ...
          >
            <div className={`${cyberPanelClass} p-5 h-full flex flex-col group-hover:bg-primary/5`}>
              <div className="flex justify-between items-start mb-3">
                <h3 className="font-bold text-lg uppercase truncate pr-4">{project.name}</h3>
                {project.url && (
          ...
          >
            <div className="flex justify-between items-start">
              <div>
                ...
                {edu.degree && <div className="text-sm opacity-80">{edu.degree}</div>}
              </div>
    ...
    <footer className="border-t border-primary/20 pt-12 mt-12 ">
      <div className="flex flex-col items-center justify-center gap-6 text-center px-4">
        <div className="space-y-1 font-mono text-xs">
          <p className="text-primary/70 mb-2 tracking-widest">SYSTEM_END_OF_LINE</p>
          <p className="opacity-50">
            © {new Date().getFullYear()} {portfolio?.name || 'My Portfolio'}. SECURE_CONNECTION.
          </p>
...
export function Cyber({ project, portfolio, themeMode, onSetThemeMode }: Props) {
